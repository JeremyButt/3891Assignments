...
ExpPtr SuperTEST = BinaryOperation::Divide(move(AddTest4), move(AddTest3));
cout << "dividing addtest4 with addtest3" << SuperTEST->evaluate() << endl;
if (SuperTEST->evaluate() == 1.115){cout << "got it bitch! "<< endl;}else{cout << " nota chance! " << endl;}
/******************
SUBTRACTION TEST
*******************/
...
*******************/
	...
	if (MultParseTEST->evaluate() == 20)
		...
		cout << "Test MultParseTEST: FAILED" << endl << endl;

	}
	catch(string message){cout << message<<endl;}

		try{ExpPtr AddParseTEST = Expression::Parse("4 + 5");
		cout << AddParseTEST->evaluate() << endl;
		if (AddParseTEST->evaluate() == 9)
			cout << "Test AddParseTEST: PASSED" << endl << endl;
		else
			cout << "Test AddParseTEST: FAILED" << endl << endl;
		}
		catch(string message){cout << message<<endl;}

		try{ExpPtr MultParseTEST2 = Expression::Parse("4.0 * 5");
		cout << MultParseTEST2->evaluate() << endl;
		if (MultParseTEST2->evaluate() == 20)
			cout << "Test MultParseTEST2: PASSED" << endl << endl;
		else
			cout << "Test MultParseTEST2: FAILED" << endl << endl;

		}
		catch(string message){cout << message<<endl;}

			try{ExpPtr AddParseTEST2 = Expression::Parse("4.5 + 4.5");
			cout << AddParseTEST2->evaluate() << endl;
			if (AddParseTEST2->evaluate() == 9)
				cout << "Test AddParseTEST2: PASSED" << endl << endl;
			else
				cout << "Test AddParseTEST2: FAILED" << endl << endl;
			}
			catch(string message){cout << message<<endl;}
}
